-- ============================================================
-- LYPO: complete database setup. Run the whole thing.
--
-- Supabase -> SQL Editor -> New query -> paste -> Run.
-- Safe to run even if you have run parts before.
--
-- Blocks that matter most right now:
--   service_role table access, without which anything using the service key
--     fails, including the weekly digest email;
--   save_project_page, without which building several pages at once loses
--     all but the last one; and
--   custom_domain, without which bring-your-own-domain silently 404s.
--
-- The only statements that can legitimately fail are the unique indexes on
-- slug and custom_domain, and only if duplicates already exist.
-- ============================================================

 ...
 *
 * There used to be two near-identical copies, one for the home page and one
 * for extra pages, and they drifted: only the home copy got the submission
 * fix and the tap tracking. Custom domains needed a third. They all call
 * this instead.
 */
...
export async function getPublishedProject(
  by: { slug: string } | { customDomain: string },
  ...
  const { data } =
    "slug" in by
      ? await query.eq("slug", by.slug).maybeSingle()
      : await query.eq("custom_domain", by.customDomain.toLowerCase()).maybeSingle();

  return (data as PublishedProject | null) ?? null;
}

/** The HTML for one page of a site, or null if that page does not exist. */
export function pageHtmlFor(
  project: PublishedProject,
  page: string,
): string | null {
  const pages = project.pages ?? null;
  if (page === "home") return pages?.home ?? project.html ?? null;
  const html = pages?.[page];
  return typeof html === "string" && html.trim() !== "" ? html : null;
}

/**
 * The site's own <head> tags live inside an iframe, so link unfurlers never
 * see them. Pull description and image out and surface them on the wrapper.
 */
export function extractMeta(html: string | null | undefined) {
...
/**
 * Where internal page links should point. On my-site.lypo.dev and on a
 * custom domain the pages sit at "/about"; on lypo.dev they are at
 * "/s/my-site/about".
 */
async function navBaseFor(project: PublishedProject): Promise<string> {
  const host = ((await headers()).get("host") ?? "").toLowerCase();
  const onOwnHost =
    !!project.slug &&
    (host.startsWith(`${project.slug}.`) || !host.includes("lypo"));
  return onOwnHost ? "" : `/s/${project.slug ?? ""}`;
}

export async function renderSite(project: PublishedProject, pageHtml: string) {
  // Best-effort: never blocks the render, and works before the analytics
  // migration has been applied.
  try {
    const supabase = await createClient();
    await supabase.rpc("increment_site_view", { pid: project.project_id });
  } catch {
    // analytics not migrated yet; the site still renders
  }
